...
const REGEX_ANALYZERS = [
	...
	(content, filePath) => {
		const text = stripHtmlToText(content);
		const NotARe = /\bNot an? [a-z][^.!?]{1,40}[.!]\s+[A-Z][^.!?]{1,60}[.!]/g;
		const ShortRebuttalRe = /\b[A-Z][^.!?]{4,80}[.!]\s+(No|Just)\s+[a-z][^.!?]{2,60}[.!]/g;
		let count = 0;
		let firstSample = "";
		let m;
		NotARe.lastIndex = 0;
		while ((m = NotARe.exec(text)) !== null) {
			count++;
			if (!firstSample) firstSample = m[0].trim().slice(0, 80);
		}
		ShortRebuttalRe.lastIndex = 0;
		while ((m = ShortRebuttalRe.exec(text)) !== null) {
			count++;
			if (!firstSample) firstSample = m[0].trim().slice(0, 80);
		}
		if (count < 3) return [];
