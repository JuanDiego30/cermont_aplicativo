 ...
 * Resuelve: Archivado de �rdenes completadas con validaciones
 * 
...
export class ArchiveOrder {
  ...
  async execute(dto: ArchiveOrderDto): Promise<void> {
    try {
      ...
      // 2. Verificar que ya no est� archivada
      if (order.archived) {
        throw new ArchiveOrderError(
          'La orden ya est� archivada',
          'ALREADY_ARCHIVED',
          400
        );
      }

      // 3. Verificar que est� en estado terminal (COMPLETADA o CANCELADA)
      const terminalStates = [OrderState.PAGO];
      
      if (!terminalStates.includes(order.state)) {
        throw new ArchiveOrderError(
          `Solo se pueden archivar �rdenes completadas o canceladas. Estado actual: ${order.state}`,
          'INVALID_STATE',
          400
        );
      }
