  ...
  async execute(dto: GenerateReporteMantenimientoDto): Promise<PdfResponseDto> {
    try {
      ...
      const templateData: MantenimientoPDFData = {
        ...
        tecnico: dto.incluirTecnico
          ? (mantenimiento as { tecnico?: { name?: string; email?: string } }).tecnico
            ? {
                nombre:
                  (
                    mantenimiento as {
      ...
      const baseSlug = String((mantenimiento as { titulo?: unknown }).titulo || 'mantenimiento')
        .substring(0, 20)
        .replace(/[^a-z0-9]/gi, '_');
      ...
      return this.pdfBuild.buildPdf({
        ...
        filenameOnCache: cacheKey => `reporte-mantenimiento-${baseSlug}-${cacheKey}.pdf`,
        generatorOptions: {
          format: dto.pageSize,
