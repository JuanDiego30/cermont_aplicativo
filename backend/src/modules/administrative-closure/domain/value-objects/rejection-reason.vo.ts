...
export class RejectionReason {
  ...
  static create(reason: string, comment: string): RejectionReason {
    if (
      !Object.values(RejectionReasonEnum).includes(
        reason as RejectionReasonEnum,
      )
    ) {
      throw new ValidationError(`Motivo inválido: ${reason}`, "reason");
    }

    if (!comment || comment.trim().length === 0) {
      throw new ValidationError(
        "Comentario es requerido al rechazar",
        "comment",
      );
    }
    ...
    if (comment.length > 500) {
      throw new ValidationError(
        "Comentario no puede exceder 500 caracteres",
        "comment",
      );
    }
