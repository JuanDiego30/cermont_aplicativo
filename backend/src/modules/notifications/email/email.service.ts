...
type NodemailerTestMessageInfo = Parameters<typeof nodemailer.getTestMessageUrl>[0];
...
export class EmailService implements OnModuleInit {
  ...
  private extractAddressList(info: unknown, key: 'accepted' | 'rejected'): string[] | undefined {
    if (!info || typeof info !== 'object') return undefined;
  ...
  async sendEmail(input: SendEmailInput): Promise<SendEmailResult> {
    ...
    if (!this.transporter) {
      if (nodeEnv === 'production') {
        this.logger.warn('EmailService: transporter no configurado; omitiendo envío.', {
          to,
