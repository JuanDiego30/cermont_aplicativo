...
- **Historial incorrecto en endpoint**: `GetHistorialEstadosUseCase` devolvía siempre el estado actual de la Order como `estadoNuevo` y no reflejaba la secuencia real de cambios.
...
- Se agregó `PENDIENTE` al enum `Orderstado` usado por DTOs.
...
- `ChangeOrderstadoUseCase` registra un `AuditLog` para cambios de estado principal, con `from/to/motivo`.
...
- `pnpm run lint` (OK)
- `pnpm run test -- --testPathPatterns=Orders` (OK)
...
- Soft-delete: al eliminar, la Order no aparece en listados ni búsquedas.
