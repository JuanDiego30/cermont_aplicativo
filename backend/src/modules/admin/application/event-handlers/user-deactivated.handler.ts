...
export class UserDeactivatedHandler {
  ...
  async handle(event: UserDeactivatedEvent): Promise<void> {
    try {
      ...
      this.logger.log(`Auditoría registrada para usuario desactivado: ${event.userEmail}`);
      ...
      this.logger.error(`Error procesando UserDeactivatedEvent para ${event.userEmail}`, error);
    }
  }
}
