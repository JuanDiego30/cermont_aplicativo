...
export async function createEvidenceV2(
  ...
  const isSafe = await scanWithClamAV(fileBuffer, `evidence-v2-${payload.serviceCaseId}-${uuidv4()}`);
  if (!isSafe) {
    throw new BadRequestError("Malware detected in uploaded evidence file");
  }
  ...
  const thumbnailBuffer = await sharp(fileBuffer).resize(200, 200, { fit: "cover" }).webp({ quality: 60 }).toBuffer();
