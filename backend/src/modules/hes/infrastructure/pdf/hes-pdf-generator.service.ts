...
export class HESPDFGeneratorService {
  ...
  private renderClienteInfo(doc: any, cliente: any): void {
    doc.fontSize(14).font('Helvetica-Bold').text('INFORMACIÓN DEL CLIENTE').moveDown(0.5);
  ...
  private renderServicioInfo(doc: any, hes: HES): void {
    doc.fontSize(14).font('Helvetica-Bold').text('INFORMACIÓN DEL SERVICIO').moveDown(0.5);
  ...
  private renderCondicionesEntrada(doc: any, condiciones: any): void {
    doc.fontSize(14).font('Helvetica-Bold').text('CONDICIONES DE ENTRADA').moveDown(0.5);
  ...
  private renderDiagnostico(doc: any, diagnostico: any): void {
    doc.fontSize(14).font('Helvetica-Bold').text('DIAGNÓSTICO PRELIMINAR').moveDown(0.5);

    doc.fontSize(10).font('Helvetica').text(`Descripción: ${diagnostico.getDescripcion()}`);
  ...
  private renderSeguridad(doc: any, seguridad: any): void {
    doc.fontSize(14).font('Helvetica-Bold').text('REQUERIMIENTOS DE SEGURIDAD').moveDown(0.5);
    ...
    doc.text(`Porcentaje de Completitud: ${seguridad.getPorcentajeCompletitud()}%`).moveDown();
  }
      ...
      if (firmaTecnico) {
        ...
        try {
          ...
          doc
            ...
            .text(
              `Fecha: ${firmaTecnico.getFechaHora().toLocaleDateString('es-CO')}`,
              320,
              y + 105
            );
        } catch (error) {
