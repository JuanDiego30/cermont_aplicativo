...
export class HESPDFGeneratorService {
  ...
  private renderCondicionesEntrada(doc: any, condiciones: any): void {
    ...
    doc
      ...
      .text(
        `Equipo Funcional: ${condiciones.isEquipoFuncional() ? "Sí" : "No"}`,
      );
  ...
  private renderSeguridad(doc: any, seguridad: any): void {
    ...
    doc
      .text(
        `Porcentaje de Completitud: ${seguridad.getPorcentajeCompletitud()}%`,
      )
      .moveDown();
  }
      ...
      if (firmaCliente) {
        ...
        try {
          ...
          doc
            ...
            .text(
              `Fecha: ${firmaCliente.getFechaHora().toLocaleDateString("es-CO")}`,
              50,
              y + 105,
            );
        } catch (error) {
