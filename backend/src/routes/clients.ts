...
router.get('/', requireAuth, requireRoles(['admin', 'coordinador', 'gerente']), async (req, res) => {
  try {
    ...
    if (search) {
      // En el esquema actual la columna es 'nombre', no 'nombre_empresa'
      query = query.or(`nombre.ilike.%${search}%,nit.ilike.%${search}%,contacto_principal.ilike.%${search}%`);
    }
...
router.post('/', requireAuth, requireRoles(['admin', 'coordinador']), async (req, res) => {
  try {
    ...
    if (!cliente.nombre_empresa || !cliente.nit) return res.status(400).json({ error: 'nombre_empresa y nit son requeridos' });
    const { data: exist, error: existErr } = await supabaseAdmin.from('clientes').select('id').eq('nit', cliente.nit).maybeSingle();
    if (existErr) return res.status(500).json({ error: 'Error al validar nit' });
    if (exist) return res.status(400).json({ error: 'NIT ya registrado' });
    const { data, error } = await supabaseAdmin.from('clientes').insert(cliente).select().single();
    if (error) return res.status(400).json({ error: 'Error al crear cliente' });
...
router.patch('/:id', requireAuth, requireRoles(['admin', 'coordinador']), async (req, res) => {
  ...
  if (error) return res.status(400).json({ error: 'Error al actualizar cliente' });
  return res.json({ data });
});
...
router.delete('/:id', requireAuth, requireRoles(['admin', 'coordinador']), async (req, res) => {
  ...
  if (eq && eq.length > 0) return res.status(400).json({ error: 'No se puede eliminar: cliente con equipos asociados' });
  const { data, error } = await supabaseAdmin.from('clientes').update({ activo: false }).eq('id', id).select().single();
  if (error) return res.status(400).json({ error: 'Error al eliminar cliente' });
  return res.json({ data });
});
