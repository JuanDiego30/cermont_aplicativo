...
router.get('/', requireAuth, async (req, res) => {
  const ordenId = req.query.orden_id as string;
  if (!ordenId) return res.status(400).json({ error: 'orden_id requerido' });
  const { data, error } = await supabaseAdmin.from('evidencias').select('*').eq('orden_id', ordenId).order('created_at', { ascending: false });
  if (error) return res.status(500).json({ error: 'Error al listar evidencias' });
...
router.post('/', requireAuth, upload.single('file'), async (req, res) => {
  try {
    ...
    if (!file || !orden_id) return res.status(400).json({ error: 'file y orden_id son requeridos' });
    ...
    if (upErr) return res.status(500).json({ error: 'Error subiendo archivo' });
    ...
    const { data, error } = await supabaseAdmin
      ...
      .select()
      .single();
    if (error) return res.status(500).json({ error: 'Error guardando evidencia' });
...
router.delete('/:id', requireAuth, async (req, res) => {
  ...
  try {
    ...
    if (evErr || !ev) return res.status(404).json({ error: 'Evidencia no encontrada' });
    ...
    if (error) return res.status(400).json({ error: 'Error al eliminar evidencia' });
    return res.json({ data });
  } catch (e) {
    console.error(e);
    return res.status(500).json({ error: 'Error interno del servidor' });
