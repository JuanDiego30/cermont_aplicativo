...
export class Monto {
  ...
  static create(value: number): Monto {
    ...
    if (value > 999999999) throw new Error('Monto excede límite máximo permitido');
...
export class OrdenNumero {
  ...
  static create(value: string): OrdenNumero {
    ...
    if (!/^ORD-\d{6,}$/.test(normalized)) throw new Error('Formato inválido: debe ser ORD-XXXXXX');
