...
function startsWithHex(buffer: Buffer, expectedHex: string): boolean {
	return buffer.subarray(0, expectedHex.length / 2).toString("hex") === expectedHex;
}

function isOfficeZip(buffer: Buffer): boolean {
	const header = buffer.subarray(0, 4).toString("hex");
	return OFFICE_ZIP_SIGNATURES.includes(header);
}
...
export const processUploadedFile = async (
	...
	try {
		...
		// Step 1: Validate binary signature against declared MIME
		validateFileSignature(req.file.buffer, req.file.mimetype, req.file.originalname);
		...
		if (!isSafe) {
			...
			throw new BadRequestError("File failed security scan", "FILE_MALWARE_DETECTED");
		}

		// Step 3: Process image (compress, optimize)
		...
		// Step 6: Attach metadata to request
