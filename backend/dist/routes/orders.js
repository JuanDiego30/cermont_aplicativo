...
router.post('/', requireAuth, async (req, res) => {
    try {
        const role = req.userRole;
        if (!canCreate(role))
            return res.status(403).json({ error: 'Sin permisos para crear órdenes' });
...
router.delete('/:id', requireAuth, async (req, res) => {
    try {
        const role = req.userRole;
        if (!canDelete(role))
            return res.status(403).json({ error: 'Sin permisos para eliminar órdenes' });
        const id = req.params.id;
...
router.post('/:id/assign', requireAuth, async (req, res) => {
    try {
        const role = req.userRole;
        if (!canAssign(role))
            return res.status(403).json({ error: 'Sin permisos para asignar órdenes' });
        const id = req.params.id;
        const { tecnico_id, fecha_programada } = req.body || {};
        if (!tecnico_id)
            return res.status(400).json({ error: 'El campo tecnico_id es requerido' });
        ...
        if (tecnico.rol !== 'tecnico')
            return res.status(400).json({ error: 'El usuario seleccionado no es un técnico' });
        const updateData = { tecnico_asignado_id: tecnico_id, estado: 'asignada', updated_at: new Date().toISOString() };
        if (fecha_programada)
...
router.patch('/:id/status', requireAuth, async (req, res) => {
    try {
        ...
        if (!allowed)
            return res.status(403).json({ error: 'Sin permisos para cambiar el estado de esta orden' });
