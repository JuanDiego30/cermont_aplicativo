...
router.post('/', requireAuth, requireRoles(['admin', 'coordinador']), async (req, res) => {
    try {
        ...
        if (!equipo.cliente_id || !equipo.numero_serie)
            return res.status(400).json({ error: 'cliente_id y numero_serie requeridos' });
        const { data: exist } = await supabaseAdmin.from('equipos').select('id').eq('numero_serie', equipo.numero_serie).maybeSingle();
        if (exist)
            return res.status(400).json({ error: 'Número de serie ya existe' });
        const { data, error } = await supabaseAdmin.from('equipos').insert(equipo).select().single();
        if (error)
            return res.status(400).json({ error: 'Error al crear equipo' });
...
router.get('/:id', requireAuth, async (req, res) => {
    ...
    const { data, error } = await supabaseAdmin
        .from('equipos')
        .select('*, cliente:clientes(*), ordenes:ordenes_trabajo(*)')
        .eq('id', id)
        .single();
    if (error || !data)
...
router.patch('/:id', requireAuth, requireRoles(['admin', 'coordinador']), async (req, res) => {
    ...
    if (error)
        return res.status(400).json({ error: 'Error al actualizar equipo' });
    return res.json({ data });
});
router.delete('/:id', requireAuth, requireRoles(['admin', 'coordinador']), async (req, res) => {
    ...
    if (ord && ord.length > 0)
        return res.status(400).json({ error: 'No se puede eliminar: equipo con órdenes asociadas' });
    const { data, error } = await supabaseAdmin.from('equipos').delete().eq('id', id).select().single();
    if (error)
        return res.status(400).json({ error: 'Error al eliminar equipo' });
    return res.json({ data });
});
