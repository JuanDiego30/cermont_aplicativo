...
|----|--------|---------|-----------|--------|-------------|---------|
| DEF-001 | Auth | Flujo de recuperación de contraseña: gateway devuelve éxito aunque solo escriba en logs; EMAIL_ENABLED=false simula envío; ausencia de proveedor activa fallback silencioso; controller ignora resultado de sendResetPasswordEmail(); no hay evidencia de entrega SMTP/sandbox; E2E no ejecutado; snapshot sin CI | Crítico | `partial` | nodemailer instalado, env.FRONTEND_URL validado, token leído de searchParams, handler creado, ruta añadida a DEDICATED_AUTH_ROUTES pero gateway y controller permiten falso éxito | token generado criptográficamente, almacenado como hash, expira en 1h, invalida sesiones vía tokenVersion. Pero el envío real puede fallar silenciosamente sin que el usuario lo sepa |
