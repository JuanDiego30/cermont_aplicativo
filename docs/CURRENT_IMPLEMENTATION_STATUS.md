...
|----|--------|----------|-------------|--------|
| DEF-001 | Auth | P0 | Password recovery flow: nodemailer not in deps, FRONTEND_URL via process.env, reset-password token not extracted from URL, missing dedicated Next.js handler | ✅ Verified |
...
| Gap | Path | Impact |
|-----|------|--------|
| Missing module specs | `docs/modules/` | No per-module technical documentation; directory missing |
| Empty security docs | `docs/security/` | Directory exists but is empty |
| Missing workflow doc | `docs/domain/SERVICE_CASE_WORKFLOW.md` | No standalone 14-step state machine workflow document |
| Outdated route map | `docs/architecture/FRONTEND_ROUTE_MAP.md` | 83 routes claimed; many are shells |
| Outdated endpoint matrix | `docs/architecture/API_ENDPOINT_MATRIX.md` | 100 documented vs 389+ real |
| Stale development status | `docs/DEVELOPMENT_STATUS.md` | 35 modules listed as `done` — over-optimistic |
