...
> Aporte único preservado en el masterplan: Metodología contract-first aplicada a 25 módulos, templates de módulo consistentes, 18 sprints, marco legal Colombia, matriz anti-duplicidad.

---
# CERMONT Contract-First Implementation Masterplan v6

**Versión:** 6.0.0  
**Fecha:** 2026-07-08  
**Tipo:** Documento de Arquitectura e Implementación Contract-First  
...
**Propósito:** Ordenar el desarrollo completo del aplicativo CERMONT bajo metodología contract-first (shared-types → domain → DB → backend → frontend → runtime → tests → docs), cubriendo 25 módulos empresariales, 14 fases operativas, costos/ERP Colombia, evidencias profesionales, trabajo offline, portal cliente y despliegue VPS.

**Regla base:** Este documento NO es código. Es un plan de implementación. El modelo programador ejecutará cada sprint/módulo en orden, un ticket a la vez, sin saltar pasos NI duplicar funcionalidad existente.
...
git checkout -b sprint-N-{nombre}
...
# Fin de sprint
git checkout deploy/vps-clean
git merge sprint-N-{nombre} --no-ff
git push
git tag sprint-N-complete
git push --tags
```

### 40.2 Conventional Commits (obligatorio)
...
### 40.3 Estructura de evidencias

```
.sisYphus/
  plans/
    cermont-contract-first-implementation-masterplan-v6.md
  evidence/
    sprint-0/
      source-alignment.md
      runtime-endpoints.md
      page-screenshots/
        ...
        ...
    sprint-1/
      task-T01-01-client-schema.png
      ...
```

### 40.4 Pull Request template
...
- {Archivo creado}: {qué se creó}

### Gates
- typecheck: ✅
- lint: ✅
- test backend: ✅ (N tests)
- test frontend: ✅ (N tests)
- build: ✅
- contracts:check: ✅
...
## 36. Estrategia de Ejecución por Sprint (Detalle)

### 36.1 Sprint 0 — Runtime Alignment (1 día)
**Objetivo:** Establecer baseline verificable del sistema antes de cualquier modificación.

**Actividades:**
1. Verificar que el repositorio está en el commit y branch correcto
2. Verificar que npm ci está actualizado
3. Verificar que MongoDB corre en 127.0.0.1:27017
4. Verificar que backend responde en localhost:4000/api/health
5. Verificar que frontend compila y carga en localhost:3000
6. Capturar screenshots de 10 páginas clave (desktop + mobile)
7. Capturar network logs de páginas clave (sin 404/500)
8. Capturar console logs de páginas clave (sin errores)
9. Ejecutar typecheck, lint, test, build, contracts:check
10. Documentar en source-alignment.md

**Comandos críticos:**
```powershell
net start MongoDB 2>$null
npm run dev -w backend 2>&1 | select -first 20
npm run dev -w frontend 2>&1 | select -first 20
npm run verify
...
**Evidencia produce:**
- `.sisyphus/evidence/s0/source-alignment.md`
- `.sisyphus/evidence/s0/runtime-endpoints.md`
- `.sisyphus/evidence/s0/browser/` (screenshots)
- `.sisyphus/evidence/s0/gates-results.txt`

### 36.2 Sprint 1 — Customers + WR + Site Visits (3 días)
**Objetivo:** Completar el módulo de clientes con datos tributarios Colombia, mejorar work requests y site visits.

**Orden de implementación:**
1. shared-types: Extender ClientSchema con taxId, taxRegime, billingEmail, etc.
2. domain: Crear client.rules.ts con validación NIT
3. backend: Extender client service + endpoints
4. frontend: Mejorar formulario cliente con campos tributarios
5. Repetir para WorkRequest: channel, priority, requestCode auto-generado
6. Repetir para SiteVisit: measurements, findings estructurados

**Parallelización posible:**
- Client backend + Client frontend (paralelo con WR schema)
- SiteVisit backend + SiteVisit frontend (secuencial a WR)
...
### 36.3 Sprint 2 — Proposals + PO + Orders (4 días)
**Objetivo:** Completar el flujo de propuesta económica, orden de compra y service case central.

**Orden:**
1. shared-types: ProposalSchema con versionado + costBaselineSnapshot
2. domain: Reglas de aprobación y conversión a orden
3. backend: Endpoints de aprobar y convertir propuesta
4. frontend: Wizard de propuesta con ítems + labor + materials + taxes
5. PO: Validación PO vs Proposal, alertas si discrepancy
6. ServiceCase: Timeline visual de 14 pasos, blockers visibles

### 36.4 Sprint 3 — Planning Contract-First (5 días)
**SPRINT CRÍTICO — Mayor brecha funcional del sistema.**

**Orden detallado:**
1. shared-types: Verificar que PlanningPacketSchema tenga los 28+ campos
2. domain: Reglas de readiness (score, blockers, next actions)
3. backend: Extender planning-packet service con readiness calculation
4. backend: Endpoint apply-kit-to-planning
5. frontend: PlanningWizard — Sección 1-2 (datos generales + cronograma)
6. frontend: PlanningWizard — Sección 3-4 (crew + materiales)
7. frontend: PlanningWizard — Sección 5-6 (herramientas + equipos)
8. frontend: PlanningWizard — Sección 7-8 (EPP + AST/PTW)
9. frontend: PlanningWizard — Sección 9-10 (cost baseline + firmas)
10. frontend: Readiness dashboard

### 36.5 Sprint 4 — Kits / Tools / Equipment (3 días)
1. shared-types: Extender ToolSchema con serial, placa, marca, modelo, estado
2. shared-types: Extender KitSchema con versionado, cost baseline, evidencias
3. backend: apply-kit-to-planning, kit readiness
4. frontend: Kit builder con imágenes
5. frontend: Catalog browser para tools/equipment

### 36.6 Sprint 5 — Forms / Checklists / CCTV / Lifelines (4 días)
1. shared-types: Extender FormTemplate y Checklist
2. backend: Template service, submission service
3. frontend: SectionedFormRenderer con foto por campo
4. frontend: CCTV form renderer (todos los campos del formato físico)
5. frontend: Lifelines form renderer (todos los componentes C/NC/NA)
6. frontend: Cálculo de concepto final automático

### 36.7 Sprint 6 — SGSST / AST / HES (3 días)
1. shared-types: SafetyAnalysisSchema completo con campos AST
2. shared-types: CertificationSchema
3. domain: Reglas de bloqueo por certificación vencida
4. backend: AST service, certification verification
5. backend: HES approval workflow
6. frontend: AST form con socialización y firmas
7. frontend: Certification dashboard con alertas

### 36.8 Sprint 7 — Execution Offline (5 días)
**SPRINT CRÍTICO — Corazón operativo del sistema.**

1. shared-types: Verificar ExecutionSessionSchema
2. backend: Sync endpoint con idempotency
3. backend: Conflict resolution (versión, diff, merge)
4. frontend: Execution wizard offline-first
5. frontend: Sync queue UI (estado, progreso, errores)
6. frontend: Conflict resolution UI
7. frontend: Indicador offline/online
8. E2E: Flujo offline → sync → online

### 36.9 Sprint 8 — Evidences / Files (4 días)
1. shared-types: Extender EvidenceSchema con metadatos completos
2. backend: File upload con hash SHA-256, thumbnails
3. frontend: Galería profesional con thumbnails + metadatos
4. frontend: Carga múltiple + cámara
5. frontend: Exportación ZIP por grupo
6. E2E: Upload → gallery → download

### 36.10 Sprint 9 — Reports + Delivery + Signatures (4 días)
1. backend: Generación de PDF con pdf-lib
2. frontend: Report wizard con selección de evidencias
3. frontend: PDF preview + descarga
4. frontend: Delivery record wizard
5. frontend: Firma digital del cliente
6. E2E: Execution → Report → Delivery → Signature

### 36.11 Sprint 10 — SES + Invoices + Payments (5 días)
**SPRINT CRÍTICO — Cierre administrativo completo.**

1. shared-types: Extender InvoiceSchema con campos DIAN
2. shared-types: TaxConfig por cliente (IVA, retefuente, ICA)
3. backend: Generación de XML factura electrónica
4. backend: Validación SES → Invoice (valores coincidentes)
5. frontend: SES wizard con aprobación
6. frontend: Invoice form con campos DIAN
7. frontend: Payment form con conciliación
8. frontend: Timeline visual SES → Invoice → Payment
9. E2E: SES → Invoice → Payment completo

### 36.12 Sprint 11 — Costs / ERP / Colombia Legal (5 días)
**SPRINT CRÍTICO — Motor financiero del sistema.**

1. shared-types: CostSchema con desglose completo
2. domain: Cálculo de variance, margin, profitability
3. backend: Cost engine con baseline vs actual
4. backend: Alerta de desviación (threshold configurable)
5. backend: Exportación contable CSV
6. frontend: Cost dashboard comparativo (gráficos)
7. frontend: Margin cards por orden/cliente
8. E2E: Proposal → Planning → Execution → Costs dashboard

### 36.13 Sprint 12 — Dashboard / KPIs (4 días)
1. backend: Aggregation queries para KPIs operativos y financieros
2. backend: SSE streaming para actualización en tiempo real
3. frontend: Dashboard widgets conectados a backend real
4. frontend: Detección de cuellos de botella
5. frontend: Alertas por rol y próximas acciones

### 36.14 Sprint 13 — Fleet / Assets / Inventory (4 días)
1. frontend: Checkout/checkin wizard
2. frontend: Historial de asignaciones
3. frontend: Mantenimiento preventivo calendar
4. frontend: Inventory stock control
5. frontend: Alertas de documentos vencidos
6. backend: Conectar fleet con planning (disponibilidad)

### 36.15 Sprint 14 — Portal Cliente (3 días)
1. Verificar y reparar portal routes
2. Dashboard de cliente con KPIs propios
3. Firma digital de actas
4. Descarga de informes y facturas

### 36.16 Sprint 15 — Admin / RBAC / Audit / Notifications (4 días)
1. domain: Verificar RAM de roles completa
2. frontend: Audit viewer con filtros
3. backend: Verificar admin-backup module
4. frontend: Notification center con preferencias
5. backend: Notificaciones programadas (cron)

### 36.17 Sprint 16 — Business Documents (3 días)
1. frontend: Document hub mejorado
2. backend: Versionado de documentos
3. backend: Ciclo de aprobación documental

### 36.18 Sprint 17 — E2E 14 Pasos + VPS (5 días)
**SPRINT FINAL — Validación completa + producción.**

1. E2E Playwright: Flujo completo WorkRequest → Payment → CLOSED
2. E2E Playwright: Offline execution → sync → verify
3. E2E Playwright: Costs proposal vs actual
4. E2E Playwright: RBAC verificación por rol
5. Performance: Lighthouse ≥ 80
6. docker-compose.yml para producción
7. Nginx + HTTPS (Let's Encrypt)
8. PM2 configuración
9. Backups automáticos diarios
10. Scripts: deploy, rollback, backup, restore
...
## Apéndice L — Estrategia de Commits

### L.1 Por ticket

```bash
git add packages/shared-types/src/schemas/new-schema.ts
git add packages/domain/src/new-rules.ts
git add backend/src/models/NewModel.ts
git add backend/src/modules/feature/feature.service.ts
git add backend/src/modules/feature/feature.controller.ts
git add backend/src/modules/feature/feature.routes.ts
git add frontend/src/modules/feature/queries.ts
git add frontend/src/modules/feature/hooks/useFeature.ts
git add frontend/src/modules/feature/ui/FeatureComponent.tsx
git commit -m "feat(contracts): add NewSchema with tax fields for Colombia"

git add backend/tests/services/feature.service.test.ts
git add frontend/tests/modules/feature.test.tsx
git commit -m "test(feature): add unit and integration tests"

git add .sisyphus/evidence/sprint-N/
git commit -m "docs(evidence): add screenshots and logs for feature"
```
...
git checkout deploy/vps-clean
git merge sprint-N-feature --no-ff
git tag sprint-N-complete
...
## Apéndice R — Pre-Implementation Checklist para el Programador

Antes de iniciar CADA sprint, el programador debe ejecutar:
...
- [ ] git pull origin deploy/vps-clean
- [ ] git checkout -b sprint-{N}-{nombre}
- [ ] git push -u origin sprint-{N}-{nombre}
...
### Documentación
- [ ] Leí las secciones relevantes del plan v6
- [ ] Leí los schemas existentes en shared-types
- [ ] Leí los módulos backend existentes
- [ ] Leí los módulos frontend existentes
...
# ¿Es un error nuevo o pre-existente?
git stash
npm run typecheck
git stash pop
```

**Regla:** Si el error es pre-existente, documentarlo y proceder. Si es nuevo, corregir antes de continuar.
...
|---|---|
| Archivo | `.sisyphus/plans/cermont-contract-first-implementation-masterplan-v6.md` |
| Líneas totales | 4100+ (verificado con Get-Content) |
| Módulos cubiertos | 25 (M00-M25) |
| Sprints creados | 18 (S0-S17) |
| Tickets definidos | 110+ (catálogo completo en Apéndice Q) |
| Apéndices incluidos | 21 (A-U) |
| Fuentes leídas | 16 archivos canónicos |
| Reglas anti-alucinación | 10 |
| DoD global | 19 puntos |
| Matriz de pesos | 25 módulos evaluados |
| Matriz anti-duplicidad | 12 reglas de herencia |
| Marco legal Colombia | 7 áreas cubiertas |
| Primer sprint recomendado | S0 — Runtime Alignment + Evidence Baseline |
| Rama del plan | `plan/contract-first-masterplan-v6` |
| Commit | `docs: add CERMONT contract-first implementation masterplan v6` |
...
1. **Este plan reemplaza v3, v4, v5 y v5.1.** Es la única fuente de verdad para la implementación. Ningún plan anterior debe ser consultado para decisiones de implementación.
2. **No empezar a codificar sin S0.** Sin baseline verificable, todo diagnóstico posterior es especulativo. S0 produce evidencia real de lo que funciona y lo que no.
3. **No saltarse el orden contract-first.** Cada módulo empieza en shared-types, termina en evidencia. No hay atajos. No hay "primero la UI y después el contrato".
4. **No duplicar funcionalidad.** Antes de crear cualquier archivo, buscar en CERMONT_CODIGO.json. Si existe schema/endpoint/página, extenderlo, no reemplazarlo.
5. **No declarar "completo" sin evidencia.** Cada punto del DoD debe verificarse con comandos específicos y screenshots. Sin evidencia, el trabajo no está completo.
6. **No modificar package.json sin ADR.** Las dependencias son sagradas. Cualquier cambio requiere Architecture Decision Record.
7. **No ignorar los gates de calidad.** typecheck, lint, test, build, contracts:check deben pasar siempre. Si fallan, detenerse y diagnosticar.
8. **Priorizar Planning (Sprint 3).** Es el módulo con mayor brecha funcional (schema rico con 28+ campos pero UI pobre con solo ReadinessGate). Sin planning maduro, execution, costs y dashboard quedan desconectados.
9. **No olvidar Colombia legal.** Facturación electrónica DIAN, CUFE, impuestos configurables, protección de datos Ley 1581, SG-SST Decreto 1072. No hay sistema CERMONT sin compliance colombiano.
10. **Evidences, Kits y Forms NO deben crearse desde cero.** YA existen como templates en cermont-form-templates.ts y kit-templates.ts. Deben mejorarse y conectarse, no reemplazarse.
11. **Execution offline es el corazón operativo.** Domain rules (223 líneas) ya existen. La brecha está en UI de sync, conflictos y estado offline visible. Sprint 7 es crítico.
12. **Respetar los 8 roles RBAC.** Los roles están definidos en `@cermont/domain`. No hardcodear strings. No crear roles nuevos sin ADR.
13. **Backward compatibility es obligatoria.** No romper endpoints existentes. Marcar deprecación, crear nuevo, migrar gradualmente.
14. **Cada sprint produce cambios visibles en localhost:3000.** Si no se ve en el navegador, no está implementado.
15. **El flujo de 14 pasos es el eje.** Cada módulo debe preguntarse: ¿cómo contribuye a completar los 14 pasos? Si no contribuye, probablemente no es necesario.
