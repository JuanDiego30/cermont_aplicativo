...
app.use((req, res, next) => {
	...
	log.info("HTTP request started", { requestId, method: req.method, path: req.originalUrl });
	res.on("finish", () => {
		log.info("HTTP request completed", { requestId, method: req.method, path: req.originalUrl, statusCode: res.statusCode, durationMs: Date.now() - startedAt, origin: req.get("origin") });
	});
	next();
});

const allowedOrigins = buildAllowedCorsOrigins({ frontendUrl: env.FRONTEND_URL, nodeEnv: env.NODE_ENV });
app.use(cors({
...
if (isTest) { app.use("/api/auth", authRoutes); } else { app.use("/api/auth", authLimiter, authRoutes); }
...
function getBackendVersion() {
	try { return JSON.parse(fs.readFileSync("package.json", "utf8")).version ?? "1.0.0"; } catch { return "1.0.0"; }
}
const backendVersion = getBackendVersion();
const memoryUsageToMb = (value) => \`\${(value / 1024 / 1024).toFixed(1)}MB\`;
...
function buildHealthPayload() {
	...
	return {
		...
		memory: { rss: memoryUsageToMb(memory.rss), heapUsed: memoryUsageToMb(memory.heapUsed), external: memoryUsageToMb(memory.external) },
