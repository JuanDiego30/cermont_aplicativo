...
(() => {
	...
	const PickerShadow = "0 16px 36px -12px oklch(0% 0 0 / 0.6)";
	...
	const ManualApplyStateTtlMs = 15 * 60 * 1000;
	...
	const HighlightTransition =
		...
		Ease +
		", opacity 150ms ease";
	const TooltipTransition = "top 140ms " + Ease + ", left 140ms " + Ease + ", opacity 150ms ease";
	...
	const Icons = {
		impeccable: `<svg ${IconAttrs}><path d="M4 20l4-1L18 9l-3-3L5 16z"/><path d="M14 7l3 3"/></svg>`,
		bolder: `<svg ${IconAttrs}><rect x="6" y="12" width="4" height="7" rx="0.5"/><rect x="14" y="5" width="4" height="14" rx="0.5"/></svg>`,
		quieter: `<svg ${IconAttrs}><rect x="6" y="5" width="4" height="14" rx="0.5"/><rect x="14" y="12" width="4" height="7" rx="0.5"/></svg>`,
		distill: `<svg ${IconAttrs}><path d="M4 5h16l-6 8v7l-4-2v-5z"/></svg>`,
		polish: `<svg ${IconAttrs}><path d="M15 3l1 3 3 1-3 1-1 3-1-3-3-1 3-1z"/><path d="M7 13l0.6 1.8 1.8 0.6-1.8 0.6-0.6 1.8-0.6-1.8-1.8-0.6 1.8-0.6z"/></svg>`,
		typeset: `<svg ${IconAttrs}><path d="M5 6h14" stroke-width="2.6"/><path d="M5 12h9" stroke-width="1.9"/><path d="M5 18h5" stroke-width="1.3"/></svg>`,
		colorize: `<svg ${IconAttrs}><circle cx="9" cy="10" r="5"/><circle cx="15" cy="10" r="5"/><circle cx="12" cy="15" r="5"/></svg>`,
		layout: `<svg ${IconAttrs}><rect x="3" y="4" width="8" height="16" rx="0.5"/><rect x="13" y="4" width="8" height="7" rx="0.5"/><rect x="13" y="13" width="8" height="7" rx="0.5"/></svg>`,
		adapt: `<svg ${IconAttrs}><rect x="2.5" y="5" width="12" height="11" rx="1"/><line x1="2.5" y1="19" x2="14.5" y2="19"/><rect x="16.5" y="8" width="5" height="11" rx="1"/></svg>`,
		animate: `<svg ${IconAttrs}><path d="M3 18c4-4 6-10 10-10"/><path d="M13 8c3 0 5 5 8 10"/><circle cx="13" cy="8" r="1.6" fill="currentColor" stroke="none"/></svg>`,
		delight: `<svg ${IconAttrs}><path d="M12 3l2 6 6 2-6 2-2 6-2-6-6-2 6-2z"/></svg>`,
		overdrive: `<svg ${IconAttrs}><path d="M13 3L5 13h5l-1 8 9-12h-6z"/></svg>`,
	};
	...
	const LiveChromeMountContract = ["root", "transport", "state", "actions"];
	const LiveUiSurfaces = [
		{
	...
	const LiveUiComponentIds = [...new Set(LiveUiSurfaces.flatMap((surface) => surface.ids))];
	...
	window.__IMPECCABLE_LIVE_CHROME_CORE__ = {
		...
		mountContract: LiveChromeMountContract,
		surfaces: LiveUiSurfaces,
		componentIds: LiveUiComponentIds,
		root: liveUiRoot,
		append: uiAppend,
	...
	function showHighlight(el) {
		...
		if (hiWasHidden) {
			...
			highlightEl.style.transition = HighlightTransition;
			highlightEl.style.opacity = "1";
			tooltipEl.style.transition = TooltipTransition;
	...
	const DragThreshold = 5; // px - below this, treat pointerup as a click
	const PinDblClickMs = 300; // two clicks on the same pin within this delete it
	...
	function onAnnotDown(e) {
		...
		if (pinWrap) {
			...
			const now = Date.now();
			if (annotLastPinClick.idx === idx && now - annotLastPinClick.time < PinDblClickMs) {
	...
	function collectManualContextPieces(rootEl, editedTexts) {
		...
		function walk(node) {
			...
			if (ManualContextSkip[tag]) return;
	...
	const BarShadowDefault = "0 4px 20px oklch(0% 0 0 / 0.08), 0 1px 3px oklch(0% 0 0 / 0.06)";
	const BarShadowUp = "0 -4px 20px oklch(0% 0 0 / 0.08), 0 -1px 3px oklch(0% 0 0 / 0.06)";
	const BarShadowDown = BarShadowDefault;
	...
	function positionBar() {
		...
		const GlobalBarReserve = 64; // global bar height + bottom margin + breathing room
		const Gap = 8;
		...
		if (belowTop + barH + Gap <= window.innerHeight - GlobalBarReserve) {
			top = belowTop;
		} else if (aboveTop >= Gap) {
			top = aboveTop;
		} else {
			top = window.innerHeight - barH - GlobalBarReserve;
		}
	...
	function placeholderSizing({ axis, parentDisplay, parentWidth, anchorFlex }) {
		...
		if (w >= PlaceholderMinWidth) return { kind: "percent" };
		return {
			kind: "explicit",
			width: Math.max(PlaceholderMinWidth, w || PlaceholderMinWidth),
		};
	}
	...
	function materializePlaceholderWidth(placeholder) {
		...
		const w = Math.max(PlaceholderMinWidth, Math.round(placeholder.offsetWidth));
		placeholder.style.flex = "";
		placeholder.style.minWidth = "";
		placeholder.style.maxWidth = "";
	...
	function clampPlaceholderSize(width, height, parentWidth) {
		const maxW = Math.max(PlaceholderMinWidth, parentWidth || PlaceholderMinWidth);
		return {
			width: Math.min(maxW, Math.max(PlaceholderMinWidth, Math.round(width))),
			height: Math.max(PlaceholderMinHeight, Math.round(height)),
		};
	}
	...
	function buildInsertPlaceholderSnapshotFromDom(anchor, placeholder) {
		return {
			...
			height: Math.round(placeholder.offsetHeight || PlaceholderDefaultHeight),
	...
	function startPlaceholderEdgeResize(edge, e) {
		...
		placeholderResizeDrag = {
			...
			parentWidth: ph.parentNode?.getBoundingClientRect().width || PlaceholderMinWidth,
	...
	function createInsertPlaceholder(anchor, position, layoutAxis) {
		...
		Object.assign(placeholder.style, {
			boxSizing: "border-box",
			height: PlaceholderDefaultHeight + "px",
	...
	function buildConfigureRow() {
		...
		voiceBtn.innerHTML = IconPageVoice;
	...
	const MixedWrapSkip = { script: 1, style: 1, template: 1, noscript: 1, svg: 1, code: 1, pre: 1 };
	...
	function hasTextRows(el) {
		...
		function check(node) {
			...
			const tag = node.tagName.toLowerCase();
			if (MixedWrapSkip[tag]) return false;
		...
		try {
			sessionStorage.setItem(
				...
				JSON.stringify({
					...
					pageUrl: location.pathname,
					updatedAt: Date.now(),
					expiresAt: Date.now() + ManualApplyStateTtlMs,
				}),
			);
		} catch {
	...
	function openTunePopover() {
		...
		if (barEl) {
			const direction = paramsPanelEl?.dataset.tuneDirection || "below";
			barEl.style.boxShadow = direction === "below" ? BarShadowUp : BarShadowDown;
		}
	...
	function closeTunePopover() {
		tuneOpen = false;
		hideParamsPanel();
		if (barEl) barEl.style.boxShadow = BarShadowDefault;
	...
	function pickVariantContent(wrapper, index) {
		...
		const NonVisual = new Set(["STYLE", "SCRIPT", "LINK", "META", "TEMPLATE"]);
		const visual = [];
		for (const child of variantDiv.children) {
			if (!NonVisual.has(child.tagName)) visual.push(child);
		}
	...
	function startScrollLock(sessionId, initialTargetY) {
		...
		window.addEventListener(
			...
			() => {
				...
				if (scrollLockTargetY == null) return;
				if (performance.now() - userGestureAt < UserGestureWindowMs) return;
	...
	let evtSource = null;
	let sseRetries = 0;
	const SseMaxRetries = 20; // generous: heartbeats keep the connection alive, so retries mean real trouble
	...
	function connectSse() {
		...
		evtSource.onerror = () => {
			sseRetries++;
			if (sseRetries <= SseMaxRetries) {
				console.log(
					"[impeccable] SSE connection lost. Retry " + sseRetries + "/" + SseMaxRetries + "...",
				);
	...
	async function inlineFontUrls(cssText) {
		...
		while ((m = urlRe.exec(cssText))) {
			if (FontExtRe.test(m[2])) urls.add(m[2]);
		}
		const map = new Map();
		await Promise.all(
			[...urls].map(async (url) => {
				try {
					...
					const ext = url.toLowerCase().match(FontExtRe)?.[1] || "woff2";
					const mime = FontMime[ext] || "application/octet-stream";
	...
	async function captureElementToBlob(el, snapshot, rect) {
		...
		try {
			...
			if (!backdrop) {
				...
				return { blob, paper: ShaderPaperFallback };
			}
	...
	const ShaderVs = `attribute vec2 a_position;
attribute vec2 a_uv;
varying vec2 v_uv;
void main() {
  v_uv = a_uv;
  gl_Position = vec4(a_position, 0.0, 1.0);
	...
	const ShaderFs = `precision highp float;
...
void main() {
  ...
  gl_FragColor = vec4(mix(ground, u_accent, dotAmt), tex.a);
	...
	function averageRgb01(ctx, w, h) {
		...
		return n ? [r / n / 255, g / n / 255, b / n / 255] : ShaderPaperFallback;
	}
	...
	async function showShaderOverlay(el, blob, rect, paper) {
		...
		let program, texture;
		try {
			const vs = compileShader(gl, gl.VERTEX_SHADER, ShaderVs);
			const fs = compileShader(gl, gl.FRAGMENT_SHADER, ShaderFs);
		...
		function frame() {
			...
			gl.uniform3f(uAccent, ShaderAccent[0], ShaderAccent[1], ShaderAccent[2]);
	...
	const SteerPageFocusPauseMs = 500;
	let detectActive = false;
	let detectScanSeq = 0;
	let activeDetectScanId = null;
	let pendingDetectScanId = null;
	const DetectEmptyMessage = "No detector issues found.";
	const PickPrefsKey = "impeccable-live-pick";
	const InteractionPrefsKey = "impeccable-live-interaction";
	const PlaceholderDefaultHeight = 80;
	const PlaceholderMinHeight = 48;
	const PlaceholderMinWidth = 120;

	function loadInteractionPrefs() {
		try {
			const raw = localStorage.getItem(InteractionPrefsKey);
			if (raw) {
			...
			const legacy = localStorage.getItem(PickPrefsKey);
			if (legacy) {
	...
	function saveInteractionPrefs() {
		try {
			localStorage.setItem(InteractionPrefsKey, JSON.stringify({ pickActive, insertActive }));
		} catch {
			/* ignore */
		}
	}
	...
	let voiceCtx = null;
	const PageChatCollapsedW = "88px";
	const PageChatProcessingW = "76px";
	const SteerAwaitTimeoutMs = 120000;
	const AgentStatusPollMs = 5000;
	const AgentDisconnectedMark = "oklch(56% 0.032 82 / 0.78)";
	const AgentDisconnectedTip = "Agent disconnected - run live-poll.mjs to connect";
	const GlobalBarSectionGap = 8;
	const GlobalBarInnerGap = 2;
	const GlobalBarInnerPadLeft = 2;
	const PageChatExpandedW = "min(280px, 38vw)";
	const IconPageChat =
	...
	function notePagePointerDown(e) {
		...
		steerFocusPauseUntil = performance.now() + SteerPageFocusPauseMs;
	...
	function preparePageChatInputForTyping() {
		...
		pageChatEl.style.width = PageChatExpandedW;
	...
	function unlockSteerChat(opts) {
		...
		pageChatEl.style.width = keepExpanded ? PageChatExpandedW : PageChatCollapsedW;
	...
	function collapsePageChat(opts) {
		...
		pageChatEl.style.width = PageChatCollapsedW;
		pageChatEl.style.cursor = "pointer";
		if (blur) {
			pageChatInput.blur();
			pageChatInput.style.pointerEvents = "none";
	...
	function initPageChat(parent, P) {
		pageChatEl = el("div", {
			...
			alignItems: "center",
			height: "28px",
			margin: "0 4px 0 " + (GlobalBarSectionGap - GlobalBarInnerGap) + "px",
			borderRadius: "7px",
		...
		chatIcon.innerHTML = IconPageChat;
		...
		pageChatVoiceBtn.innerHTML = IconPageVoice;
	...
	function syncAgentPollingUi(connected) {
		...
		if (mark) {
			mark.innerHTML = brandMarkSvg(connected ? P.accent : AgentDisconnectedMark, 18);
	...
	function ensureAgentPollTooltip() {
		...
		agentPollTooltipEl.textContent = AgentDisconnectedTip;
	...
	function startAgentStatusPoll() {
		...
		agentStatusPollTimer = setInterval(fetchAgentPollingStatus, AgentStatusPollMs);
	}
	...
	function initGlobalBar() {
		...
		const brand = el("span", {
			...
			alignSelf: "stretch",
			position: "relative",
			padding: "0 " + (GlobalBarSectionGap - GlobalBarInnerPadLeft) + "px 0 14px",
			background: "transparent",
			color: P.accent,
			flexShrink: "0",
		});
		...
		const inner = el("div", {
			display: "flex",
			alignItems: "center",
			padding: "4px 5px 4px " + GlobalBarInnerPadLeft + "px",
			gap: GlobalBarInnerGap + "px",
		});
	...
	function onDetectMessage(e) {
		...
		if (e.data.source === "impeccable-results") {
			...
			if (detectActive && pendingDetectScanId && detectCount === 0) {
				showToast(DetectEmptyMessage, 3200);
			}
			pendingDetectScanId = null;
			updateGlobalBarState();
		}
	...
	const DesignPrefsKey = "impeccable-live-design-panel";
	const DesignPanelWidth = 440;
	...
	function loadDesignPrefs() {
		...
		try {
			const raw = localStorage.getItem(DesignPrefsKey);
			if (!raw) return;
			const prefs = JSON.parse(raw);
	...
	function designPanelCss(Bp) {
      ...
      .panel {
        ...
        width: ${DesignPanelWidth}px; max-width: calc(100vw - 24px);
        background: ${Bp.surface};
