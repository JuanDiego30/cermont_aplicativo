...
(() => {
	...
	const ScrollKeySuffix = "-scroll";
	function writeScrollY(y) {
		try {
			localStorage.setItem(LsKey + ScrollKeySuffix, String(y));
		} catch {}
	}
	function readScrollY() {
		try {
			const raw = localStorage.getItem(LsKey + ScrollKeySuffix);
			if (raw == null) {
				return null;
			}
	...
	function clearScrollY() {
		try {
			localStorage.removeItem(LsKey + ScrollKeySuffix);
		} catch {}
	}
	...
	function closeTunePopover() {
		...
		hideParamsPanel();
		if (barEl) {
			barEl.style.boxShadow = BarShadowDefault;
		}
	...
	function pickVariantContent(wrapper, index) {
		...
		for (const child of variantDiv.children) {
			if (!NonVisual.has(child.tagName)) {
				visual.push(child);
			}
		}
		if (visual.length === 1) {
	...
	function startScrollLock(sessionId, initialTargetY) {
				...
				if (m.target?.closest?.(`[data-impeccable-variants="${sessionId}"]`)) {
					const childAdds = Array.from(m.addedNodes)
						.map((n) =>
							n.nodeType === 1
								? n.tagName +
									(n.dataset?.impeccableVariant ? `[variant=${n.dataset.impeccableVariant}]` : "")
								: n.nodeType,
						)
						.join(",");
		...
		window.addEventListener(
			...
			() => {
				...
				if (performance.now() - userGestureAt < UserGestureWindowMs) {
					return;
				}
	...
	function connectSse() {
		...
		evtSource.onerror = () => {
			...
			if (sseRetries <= SseMaxRetries) {
				console.log(`[impeccable] SSE connection lost. Retry ${sseRetries}/${SseMaxRetries}...`);
	...
	function handleKeyDown(e) {
		...
		if (annotEditing?.input && e.target === annotEditing.input) {
			return;
		}
		if (e.key === "Escape") {
			e.preventDefault();
...
void main() {
  ...
  gl_FragColor = vec4(mix(base, dotLayer, band), 1.0);
	...
	const ShaderAccent = [0.82, 0.16, 0.47];
	...
	function markSessionHandled() {
		...
		try {
			localStorage.setItem(`${LsKey}-handled`, currentSessionId);
		} catch {}
	}

	function isSessionHandled(id) {
		try {
			return localStorage.getItem(`${LsKey}-handled`) === id;
		} catch {
			return false;
		}
	}
	...
	function clearHandled() {
		try {
			localStorage.removeItem(`${LsKey}-handled`);
		} catch {}
	}
