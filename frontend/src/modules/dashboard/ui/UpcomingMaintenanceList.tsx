...
import { formatMaintenanceKitActivityLabel } from "@/modules/maintenance/constants";
	...
	return (
		...
		>
			<div className="mb-5 flex items-center justify-between gap-3">
				<div>
					<h3 id={headingId} className="text-xl font-semibold text-[var(--text-primary)]">
			...
			<ul className="mb-3 flex flex-col gap-2">
				{items.map((item) => {
					const updatedAt = item.updatedAt ? formatDate(item.updatedAt, "dd MMM") : ",";
					...
					return (
						...
						>
							<div className="relative flex size-12 shrink-0 items-center justify-center rounded-full bg-[var(--surface-primary)] shadow-[var(--shadow-1)]">
								<Wrench aria-hidden="true" className="size-5 text-[var(--color-brand-blue)]" />
							</div>
							...
							<div className="min-w-0 flex-1">
								...
								<div className="mt-0.5 flex flex-col gap-1 sm:flex-row sm:items-center sm:gap-3">
									...
									<span className="truncate text-xs text-[var(--text-tertiary)]">
										{toolCount} herramientas · {equipmentCount} equipos
									</span>
								</div>
							</div>
							...
							<div className="flex shrink-0 items-center gap-2">
								...
								<span className="rounded-full bg-[var(--color-info-bg)] px-3 py-1 text-xs font-semibold text-[var(--color-brand-blue)]">
									{updatedAt}
								</span>
							</div>
						</li>
