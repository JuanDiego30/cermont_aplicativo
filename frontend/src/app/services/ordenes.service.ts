...
export class OrdenesService {
  ...
  getAll(params?: QueryOrdenParams): Observable<PaginatedResponse<Orden>> {
    return this.http
      .get<
        PaginatedResponse<Orden>
      >(this.API_URL, { params: buildHttpParams(params as unknown as Record<string, unknown> | undefined) })
  ...
  getById(id: string): Observable<Orden> {
    return this.http.get<Orden>(`${this.API_URL}/${id}`).pipe(catchError(this.handleError));
  }
  ...
  create(orden: CreateOrdenDto): Observable<Orden> {
    return this.http.post<Orden>(this.API_URL, orden).pipe(catchError(this.handleError));
  }
  ...
  update(id: string, orden: UpdateOrdenDto): Observable<Orden> {
    return this.http
      .patch<Orden>(`${this.API_URL}/${id}`, orden)
      .pipe(catchError(this.handleError));
  }
  ...
  getEstadisticas(): Observable<Record<string, unknown>> {
    return this.http
      .get<Record<string, unknown>>(`${this.API_URL}/stats`)
      .pipe(catchError(this.handleError));
  }
