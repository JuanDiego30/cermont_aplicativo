...
% Versión refactorizada y expandida bajo el paradigma "Opus 4 Thinking"
...
\bottomrule
\endlastfoot
1  & Solicitud formal & Recepción del requerimiento técnico del cliente por correo, llamada o canal autorizado. & WorkRequest\\
2  & Visita técnica & Levantamiento de información, aclaración de dudas, mediciones y registro fotográfico cuando aplica. & SiteVisit\\
3  & Propuesta económica & Preparación de la oferta técnica y económica, con alcance, recursos y condiciones para aprobación del cliente. & Proposal\\
4  & Aprobación con PO & Recepción de la orden de compra o autorización formal que habilita la ejecución. & PurchaseOrder\\
5  & Planeación & Definición del cronograma, mano de obra, herramientas, equipos, certificaciones, AST y documentos de apoyo. & PlanningPacket\\
6  & Ejecución & Desarrollo de la actividad con permisos, socialización de AST, checklist, procedimientos y registro fotográfico. & ExecutionSession\\
7  & Informe técnico & Consolidación de actividades realizadas, observaciones, evidencias y registro fotográfico. & TechnicalReport\\
8  & Acta de entrega & Elaboración y envío del acta final al cliente para revisión y firma. & DeliveryRecord\\
9  & Acta firmada & Recepción del acta firmada o del soporte de aceptación del cliente. & ClientAcceptance\\
10 & SES / Ariba & Elaboración de la hoja de entrada de servicio y envío para aprobación en la plataforma correspondiente. & ServiceEntrySheet\\
11 & SES aprobada & Recepción de la aprobación de la SES por parte del cliente. & SESApproval\\
12 & Factura & Elaboración y envío de factura para aprobación del cliente por la plataforma habilitada. & InvoiceTracking\\
13 & Aprobación de factura & Recepción de aprobación de la factura por parte del cliente. & InvoiceApproval\\
14 & Pago & Registro, conciliación y cierre administrativo del pago del servicio. & PaymentRecord\\
\end{longtable}
\end{center}
...
\begin{tikzpicture}[
    ...
    bloque/.style={draw, rounded corners, align=center, fill=gray!8, minimum height=0.8cm},
    flecha/.style={-{Latex[length=2mm]}, line width=0.8pt},
...
\begin{itemize}
    ...
    \item \textbf{Respuesta del aplicativo:} Motor de renderizado y exportación automática a formato PDF (\texttt{pdf-lib}) que toma los datos transaccionales, las respuestas de checklist validadas y las evidencias fotográficas almacenadas en base de datos para compilar de manera instantánea el informe técnico preliminar conforme a las plantillas corporativas de CERMONT S.A.S.
\end{itemize}
...
\begin{figure}[!htbp]
\centering
\begin{adjustbox}{max width=\linewidth,max totalheight=0.34\textheight,keepaspectratio}
\begin{tikzpicture}[
    every node/.style={transform shape},
    bloque/.style={draw, rounded corners, align=center, fill=gray!8},
    bloqueCritico/.style={draw=institucionalRed, fill=institucionalRed!10, rounded corners, align=center, font=\scriptsize\bfseries},
    flecha/.style={-{Latex[length=2mm]}, line width=0.8pt},
    faseBox/.style={bloque, text width=3.0cm, minimum height=1.0cm},
    flechaSecundaria/.style={flecha},
    flechaCritica/.style={-{Latex[length=3mm]}, line width=1.6pt, draw=institucionalRed}
]
    \node (f1) [faseBox] at (0,1.0) {Fase 1: Planeación\\ \tiny Recursos no normalizados.};
    \node (f2) [faseBox] at (4.4,1.0) {Fase 2: Ejecución\\ \tiny Papel, fotos sin contexto y sin red.};
    \node (f3) [faseBox] at (0,-0.2) {Fase 3: Informes y Actas\\ \tiny Recaptura y retraso documental.};
    \node (f4) [faseBox] at (4.4,-0.2) {Fase 4: Cierre Admin.\\ \tiny Soportes incompletos y retrasos de facturación.};

    \node (prob) [bloqueCritico, text width=4.2cm, minimum height=1.0cm] at (2.2,-1.5) {Fragmentación documental y atraso en cierres};

    \draw [flechaSecundaria] (f1.south) -- ++(0,-0.15) -| (prob.north west);
    \draw [flechaSecundaria] (f2.south) -- ++(0,-0.15) -| (prob.north east);
    \draw [flechaSecundaria] (f3.south) -- ++(0,-0.15) -| (prob.west);
    \draw [flechaSecundaria] (f4.south) -- ++(0,-0.15) -| (prob.east);
\end{tikzpicture}%
\end{adjustbox}
\caption{Diagrama de fallas por fases y su incidencia en el cierre operativo y documental. Fuente: Elaboración propia.}
...
Para CERMONT S.A.S., la herramienta representa un salto cualitativo hacia la madurez de procesos operativos y la digitalización integral. El aplicativo unifica en una única interfaz de trabajo a actores típicamente desconectados: gerentes que evalúan avances, ingenieros residentes que estructuran kits de planeación, técnicos de campo que recolectan evidencias y personal de seguimiento administrativo que rastrea actas, SES y facturas aprobadas. Al centralizar y auditar cada cambio de estado (audit logging), la empresa reduce costos transaccionales derivados de la búsqueda de soportes físicos y correos electrónicos, contribuye a reducir devoluciones de SES asociadas con documentación incompleta, y proporciona un soporte transparente a sus clientes corporativos, mejorando su posicionamiento competitivo como contratista confiable y digitalizado en Arauca y la región.
...
El proyecto se rige por la metodología Design Science Research (DSR), un enfoque riguroso de investigación aplicada que se centra en el diseño, desarrollo, prueba y validación de un artefacto de ingeniería real para resolver un problema práctico observado \cite{hevner2004}. El uso de esta metodología garantiza que el producto final (el aplicativo de software) sea evaluado no solo por su correcta funcionalidad interna mediante pruebas de caja negra y typechecks unitarios, sino también por su utilidad y aplicabilidad en el escenario operativo y comercial real de CERMONT S.A.S., separando con total nitidez científica las evidencias técnicas construidas de aquellas métricas operativas sujetas a medición progresiva durante la práctica profesional.

\section{Objetivos}
\label{sec:objetivos}

Para orientar el desarrollo y garantizar la solución de la problemática descrita, se formularon los siguientes objetivos:

\subsection{Objetivo General}
Desarrollar un aplicativo web que permita gestionar de forma centralizada las órdenes de trabajo y optimizar la trazabilidad de los procesos operativos y administrativos en CERMONT S.A.S., mejorando la planeación, ejecución y cierre administrativo de los servicios técnicos.
...
\begin{itemize}
    ...
    \item \textbf{Módulo de Ejecución PWA Offline:} Diligenciamiento móvil de listas de verificación y checklists técnicos en campo utilizando IndexedDB local en ausencia de cobertura celular, dotado de captura directa de evidencias fotográficas y encolamiento en segundo plano operado por Service Workers \cite{mdn_service_worker}.
...
\def\StakeholderRow#1#2#3#4{\parbox[t]{2.2cm}{\raggedright #1} & \parbox[t]{4.4cm}{\raggedright #2} & \parbox[t]{4.7cm}{\raggedright #3} & \parbox[t]{2.3cm}{\raggedright #4}\\\midrule}
\begin{tabular}{@{}l l l l@{}}
\toprule
\textbf{Actor} & \textbf{Necesidad principal} & \textbf{Problema actual} & \textbf{Beneficio esperado} \\
\midrule
\StakeholderRow{Gerente}{Visibilidad sobre el estado de todas las órdenes y la rentabilidad por servicio.}{Sin acceso a datos agregados en tiempo real; decisiones basadas en informes verbales.}{Dashboard con KPIs operativos y financieros; trazabilidad total de cada orden.}
...
\StakeholderRow{Técnico / Operador}{Captura ágil de datos en campo, incluso sin conectividad.}{Formatos físicos que se dañan o extravían; fotos en dispositivo personal sin trazabilidad.}{Aplicación PWA con operación offline; captura de evidencias vinculadas automáticamente a la orden.}
\bottomrule
\end{tabular}
\end{table}
\begin{table}[!htbp]
...
\renewcommand{\arraystretch}{0.95}
\setlength{\tabcolsep}{2pt}
\def\StakeholderRowB#1#2#3#4{\parbox[t]{2.2cm}{\raggedright #1} & \parbox[t]{4.4cm}{\raggedright #2} & \parbox[t]{4.7cm}{\raggedright #3} & \parbox[t]{2.3cm}{\raggedright #4}\\\midrule}
\begin{tabular}{@{}l l l l@{}}
\toprule
\textbf{Actor} & \textbf{Necesidad principal} & \textbf{Problema actual} & \textbf{Beneficio esperado} \\
\midrule
...
Este análisis de actores fue utilizado como insumo para la definición de los módulos del sistema y la matriz de permisos RBAC. Cada módulo del aplicativo responde a las necesidades de al menos un perfil de usuario identificado en esta matriz, garantizando que el desarrollo se orientó a resolver problemas reales de los usuarios finales y no a implementar funcionalidades sin un propósito definido.
...
% --- AMPLIACIÓN ACADÉMICA INTEGRADA CAPÍTULO 1 ---
