...
% Versión refactorizada y enriquecida con evidencia técnica real de tests
% =====================================================================
...
Para la ejecución y mantenimiento de la suite de pruebas del monorepo, se implementó un entorno de pruebas robusto basado en el framework \textbf{Vitest v4.1.5}, el cual proporciona una velocidad de ejecución excepcional gracias a su integración nativa con el motor de compilación Vite. La suite de pruebas interactúa directamente con los endpoints HTTP expuestos por el backend en Express utilizando la librería \textbf{Supertest}.

\input{Figuras/tam_validacion.tex}

El entorno garantiza un aislamiento absoluto de los datos de producción aplicando los siguientes patrones técnicos en el archivo de inicialización global \texttt{setup.ts}:
\begin{itemize}
...
La ejecución automatizada documentada en el reporte técnico anexo registra una suite amplia de pruebas distribuidas en varios archivos de pruebas, sin fallos reportados en dicha corrida. Esta evidencia valida el comportamiento técnico del artefacto, pero no sustituye la medición operativa en producción con usuarios reales. La suite completa se integra de forma fluida en los ganchos de pre-commit (\textit{git hooks} con Husky y lint-staged) y en el pipeline de Integración Continua (CI/CD) local.
...
\begin{itemize}
    ...
    \item \textbf{Resultado}: Verificado en la corrida documentada. No se reportaron errores en el escenario técnico anexo.
\end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{Resultado}: Verificado en la corrida documentada. El perímetro de seguridad \texttt{proxy.ts} y los middlewares del backend bloquean accesos no autorizados en los casos evaluados.
\end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{Resultado}: Verificado en la corrida documentada. Los payloads inválidos fueron rechazados con respuesta de error controlada.
\end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{Resultado}: Verificado en la corrida documentada. Las transiciones inválidas fueron bloqueadas en los casos de prueba ejecutados.
\end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{Resultado}: Verificado en la corrida documentada. Los casos ejecutados conservaron consistencia en cálculos agregados y mapeo de fases.
\end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{Resultado}: Verificado en la corrida documentada. Se confirmó la generación de archivos PDF válidos para los escenarios probados.
\end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{Resultado}: Verificado en la corrida documentada. La evidencia técnica respalda la asociación de metadatos permitidos con la orden correspondiente.
\end{itemize}
...
\small
\def\TestRow#1#2#3#4{\parbox[t]{2.4cm}{\raggedright #1} & \parbox[t]{4.7cm}{\raggedright #2} & \parbox[t]{4.7cm}{\raggedright #3} & \parbox[t]{2.4cm}{\raggedright #4}\\\midrule}
...
\small
\def\AcepRow#1#2#3#4{\parbox[t]{3.0cm}{\raggedright #1} & \parbox[t]{5.1cm}{\raggedright #2} & \parbox[t]{4.2cm}{\raggedright #3} & \parbox[t]{2.1cm}{\raggedright #4}\\\midrule}
\begin{adjustbox}{max width=\textwidth}
\begin{tabular}{@{}l l l l@{}}
\toprule
...
La estrategia de pruebas implementada para el aplicativo web de CERMONT S.A.S. documenta la madurez del artefacto de ingeniería construido. La evidencia anexa respalda pruebas automatizadas, un modelo transaccional en memoria y validaciones Zod de extremo a extremo, lo que permite verificar la estabilidad del sistema y reducir el riesgo de regresiones de código. Este capítulo constituye una evidencia técnica relevante dentro de la tesis de grado.
...
Para garantizar que la validación del sistema cubriera todos los objetivos del proyecto, se estableció una matriz de trazabilidad que relaciona cada objetivo específico con las pruebas diseñadas para verificarlo. Esta matriz, presentada en la Tabla \ref{tab:trazabilidad-pruebas}, permite a cualquier revisor académico comprobar que cada afirmación de cumplimiento está respaldada por evidencia de pruebas.
...
\small
\def\TrzRow#1#2#3{\parbox[t]{3.0cm}{\raggedright #1} & \parbox[t]{9.0cm}{\raggedright #2} & \parbox[t]{2.5cm}{\raggedright #3}\\\midrule}
\begin{tabular}{@{}l l l@{}}
\toprule
...
Estas recomendaciones no constituyen resultados del proyecto, sino orientaciones para la continuidad del trabajo durante y después de la práctica empresarial. Su ejecución permitiría transformar los indicadores actualmente marcados como ``pendiente por medir'' en resultados cuantitativos que fortalezcan la evaluación del impacto del sistema.

% --- AMPLIACIÓN ACADÉMICA INTEGRADA CAPÍTULO 8 ---
...
\begin{longtable}{>{\raggedright\arraybackslash}p{3.0cm} >{\raggedright\arraybackslash}p{4.6cm} >{\raggedright\arraybackslash}p{4.5cm} >{\raggedright\arraybackslash}p{2.6cm}}
