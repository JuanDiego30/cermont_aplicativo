...
% Versión académica con enfoque en conceptos aplicables al proyecto
% =====================================================================
...
\def\SynthRow#1#2#3{\parbox[t]{3.0cm}{\raggedright #1} & \parbox[t]{4.2cm}{\raggedright #2} & \parbox[t]{8.0cm}{\raggedright #3}\\\hline}
...
\section{Herramientas tecnológicas utilizadas y criterio de selección}
\label{sec:herramientas-stack-teorico}

El marco teórico también debe explicar las herramientas concretas empleadas en el desarrollo, no como una lista de nombres comerciales, sino como un conjunto de decisiones técnicas asociadas al problema de CERMONT S.A.S. La Figura \ref{fig:ecosistema-tecnologico-cermont} organiza las tecnologías verificables del repositorio por capa de responsabilidad: interfaz de usuario, API, contratos, seguridad, persistencia, pruebas y soporte de despliegue. Esta lectura por capas permite defender por qué el proyecto no se abordó como un conjunto de formularios aislados, sino como una plataforma web con contratos compartidos y trazabilidad documental.

\begin{figure}[!htbp]
\centering
\begin{adjustbox}{max width=\textwidth,max totalheight=0.44\textheight,keepaspectratio}
\input{Figuras/ecosistema_tecnologico_cermont.tex}
\end{adjustbox}
\caption{Ecosistema tecnológico empleado en el aplicativo CERMONT, organizado por capas de responsabilidad. Fuente: elaboración propia con base en el repositorio del proyecto.}
\label{fig:ecosistema-tecnologico-cermont}
\end{figure}

La selección de tecnologías se realizó atendiendo restricciones académicas y empresariales: un solo desarrollador principal durante la práctica, necesidad de mantener coherencia entre frontend y backend, operación en campo con conectividad variable, manejo de documentos heterogéneos y control de permisos por rol. La Tabla \ref{tab:seleccion-stack-tecnologico} presenta una versión sintética, pensada para recrearse manualmente en Word si el documento se entrega en ese formato. Por esa razón se evitaron columnas redundantes, versiones excesivas o descripciones promocionales.

\begin{table}[!htbp]
\centering
\caption{Justificación académica del stack tecnológico seleccionado. Fuente: elaboración propia.}
\label{tab:seleccion-stack-tecnologico}
\footnotesize
\begin{adjustbox}{max width=\textwidth}
\begin{tabular}{|p{3.2cm}|p{4.3cm}|p{3.4cm}|p{4.1cm}|}
\hline\ClassicTableHeader
\textbf{Tecnología} & \textbf{Necesidad del proyecto} & \textbf{Alternativa considerada} & \textbf{Criterio de selección}\\
\hline
Next.js y React & Construir vistas protegidas, formularios y paneles de seguimiento para los módulos del flujo operativo. & React SPA con Vite o Angular. & Integración de enrutamiento, componentes y protección de rutas en una misma base frontend.\\
Node.js y Express & Exponer una API modular para órdenes, propuestas, evidencias, actas, SES y pagos. & NestJS o Fastify. & Menor complejidad estructural para un equipo pequeño y compatibilidad directa con middlewares de seguridad.\\
MongoDB y Mongoose & Almacenar órdenes, evidencias y documentos con estructuras variables según el tipo de servicio. & PostgreSQL con ORM relacional. & Modelo documental adecuado para formularios y soportes heterogéneos; exige control explícito de consistencia.\\
TypeScript & Evitar divergencias de tipos entre capas y favorecer la mantenibilidad del código. & JavaScript sin tipado estático. & Tipado estricto en frontend, backend y paquetes compartidos.\\
Zod y paquetes compartidos & Validar datos con una fuente única de verdad entre contratos, API y formularios. & Joi o interfaces duplicadas. & Permite inferir tipos desde esquemas y aplicar validación en ambos extremos.\\
JWT y RBAC & Controlar acceso a módulos sensibles según rol y estado del proceso. & Sesiones tradicionales o validación solo en interfaz. & Autenticación sin estado y autorización verificable en backend.\\
TanStack Query y Zustand & Separar estado del servidor, sesión de usuario y estados de carga o error. & \texttt{fetch} directo en componentes o stores manuales. & Mejora la organización del frontend y evita mezclar red con componentes visuales.\\
PWA, Service Worker e IndexedDB & Permitir captura y sincronización diferida en escenarios de conectividad variable. & Aplicación exclusivamente en línea. & Soporte técnico para operación offline parcial; la sincronización completa de archivos queda como línea de fortalecimiento.\\
Vitest, Supertest y Biome & Validar reglas, endpoints y calidad estática del código. & Jest, pruebas manuales o linting disperso. & Ejecución rápida y coherente con TypeScript y el monorepo.\\
npm workspaces y Docker & Organizar frontend, backend y paquetes compartidos; facilitar entornos reproducibles. & Repositorios separados o ejecución manual por carpeta. & Mantiene estructura monorepo y controla la duplicación de contratos.\\
\hline
\end{tabular}
\end{adjustbox}
\end{table}

El enfoque \textit{contract-first} resume la forma en que estas herramientas se conectan. Primero se define el contrato de datos con Zod; luego se implementan el modelo, el servicio, la ruta HTTP, el cliente de API, el hook de consulta y finalmente la pantalla. Esta secuencia, representada en la Figura \ref{fig:flujo-contract-first-cermont}, evita que el frontend y el backend evolucionen con interpretaciones diferentes sobre una misma entidad del negocio.

\begin{figure}[!htbp]
\centering
\begin{adjustbox}{max width=\textwidth,max totalheight=0.32\textheight,keepaspectratio}
\input{Figuras/flujo_contract_first_cermont.tex}
\end{adjustbox}
\caption{Flujo contract-first aplicado a los módulos del aplicativo. Fuente: elaboración propia.}
\label{fig:flujo-contract-first-cermont}
\end{figure}
...
\begin{enumerate}
    ...
    \item \textbf{Nivel 3 — Sistema digital integrado:} Una plataforma centralizada gestiona el ciclo de vida de la orden de trabajo, con módulos interconectados para planeación, ejecución, evidencias y cierre. Los datos se capturan en el punto de origen y se reutilizan entre módulos cuando existe contrato compartido. La trazabilidad se respalda mediante registros de auditoría. \textit{Este es el nivel objetivo del proyecto CERMONT.}
