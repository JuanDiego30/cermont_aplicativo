...
function extractColors(section) {
	...
	const groups = [];
	const RoleKeywords = /^(primary|secondary|tertiary|neutral|accent)\b/i;
	...
	for (const sub of subs.slice(1)) {
		...
		const allRoleBullets =
			parsed.length > 0 && parsed.every((p) => p.name && RoleKeywords.test(p.name));
	...
	if (groups.length === 0) {
		...
		if (flat.length) {
			for (const p of flat) {
				if (p.name && RoleKeywords.test(p.name)) {
					groups.push({ role: p.name, colors: [p] });
				} else {
