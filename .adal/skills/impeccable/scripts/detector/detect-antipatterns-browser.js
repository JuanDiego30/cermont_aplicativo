...
(() => {
	...
	const BorderSafeTags = new Set([...SafeTags].filter((t) => t !== "label"));
	...
	const VercelDomains = ["vercel.com", "nextjs.org", "v0.app"];
	const GithubDomains = ["github.com", "githubnext.com"];
	const BrandFontDomains = {
		roboto: GoogleDomains,
		"google sans": GoogleDomains,
		"product sans": GoogleDomains,
		geist: VercelDomains,
		"geist sans": VercelDomains,
		"geist mono": VercelDomains,
		"mona sans": GithubDomains,
	};

	function isBrandFontOnOwnDomain(font) {
		if (typeof location === "undefined") return false;
		const allowed = BrandFontDomains[font];
	...
	const WcagLargeTextPx = 18 * (96 / 72);
	const WcagLargeBoldTextPx = 14 * (96 / 72);
	...
	const DetectorIsBrowser = typeof window !== "undefined";

	// ─── Section 3: Pure Detection ──────────────────────────────────────────────

	function checkBorders(tag, widths, colors, radius) {
		if (BorderSafeTags.has(tag)) return [];
		const findings = [];
	...
	function checkColors(opts) {
		...
		if (hasDirectText && textColor && !isEmojiOnly) {
			...
			if (bgs) {
				...
				const isLargeText =
					fontSize >= WcagLargeTextPx || (fontSize >= WcagLargeBoldTextPx && fontWeight >= 700);
				...
				if (ratio < threshold) {
					...
					const isAlphaFallbackFp =
						!DetectorIsBrowser && !effectiveBg && textColor.a != null && textColor.a < 1;
	...
	function resolveSerif(fontFamily) {
		...
		const primary = tokens.find((f) => f && !GenericFonts.has(f)) || null;
		if (!primary) return { primary: null, isSerif: false };
		if (KnownSerifFonts.has(primary)) return { primary, isSerif: true };
	...
	const LayoutTransitionProps = new Set([
		"width",
		"height",
		"padding",
		"margin",
	...
	function checkMotion(opts) {
		...
		if (transitionProperty && transitionProperty !== "all" && transitionProperty !== "none") {
			...
			const layoutFound = props.filter((p) => LayoutTransitionProps.has(p));
	...
	function readOwnBackgroundColor(el, computedStyle) {
		...
		if (DetectorIsBrowser || (bg && bg.a >= 0.1)) return bg;
	...
	function resolveBackground(el, win, customPropMap) {
		...
		while (current && current.nodeType === 1) {
			const style = DetectorIsBrowser ? getComputedStyle(current) : win.getComputedStyle(current);
			...
			if (!DetectorIsBrowser && (!bg || bg.a < 0.1)) {
			...
			if (bg && bg.a > 0.1) {
				if (DetectorIsBrowser || bg.a >= 0.5) return bg;
			}
	...
	const RepeatedKickerSkipSelector = [
		"nav",
		"form",
		"table",
		"thead",
	...
	function isRepeatedKickerCandidate(opts) {
		...
		if (!kickerTag || HeadingTags.has(kickerTag)) return false;
	...
	function collectRepeatedSectionKickerCandidates(doc, getStyle, resolveLetterSpacing) {
		...
		for (const heading of doc.querySelectorAll("h2, h3, h4")) {
			if (heading.closest?.(RepeatedKickerSkipSelector)) continue;
			const kicker = heading.previousElementSibling;
			if (!kicker || kicker.closest?.(RepeatedKickerSkipSelector)) continue;
	...
	const QualityTextTags = new Set(["p", "li", "td", "th", "dd", "blockquote", "figcaption"]);
	...
	function checkQuality(opts) {
		...
		if (rect && hasDirectText && QualityTextTags.has(tag) && rect.width > 0 && textLen > lineMax) {
		...
		// visual failure mode.
		{
			...
			if (
				...
				if (anyVisible) {
					...
					const ChildInsulateThreshold = 4;
					...
					for (const child of el.children) {
						...
						for (const s of ["top", "right", "bottom", "left"]) {
							if (childPad[s] >= ChildInsulateThreshold) childrenInsulate[s] = true;
						}
					}
					...
					for (const side of ["top", "right", "bottom", "left"]) {
						...
						if (sideBounded && pad[side] <= PadThreshold && !childrenInsulate[side]) {
	...
	function checkTypography() {
		...
		for (const el of document.querySelectorAll(
			...
			const primary = stack.find((f) => f && !GenericFonts.has(f));
			if (!primary) continue;
		...
		if (totalTextElements >= 20) {
			...
			const PrimaryThreshold = 0.15;
			for (const [font, count] of fontUsage) {
				const share = count / totalTextElements;
				if (share < PrimaryThreshold) continue;
				if (!OverusedFonts.has(font)) continue;
	...
	function checkPageTypography(doc, win) {
		...
		for (const sheet of doc.styleSheets) {
			...
			for (const rule of rules) {
				...
				if (primary) {
					fonts.add(primary);
					if (OverusedFonts.has(primary)) overusedFound.add(primary);
				}
			}
		}
		...
		while ((m = gfRe.exec(html)) !== null) {
			...
			for (const f of families) {
				fonts.add(f);
				if (OverusedFonts.has(f)) overusedFound.add(f);
			}
		}
	...
	function creamFromClassList(cls) {
		...
		for (const [tok, hex] of Object.entries(TailwindBgHex)) {
	...
	// alone, is the tell.
	const OversizedH1FontPx = 72;
	const OversizedH1MinChars = 40;
	function checkOversizedH1({ tag, fontSize, headingText }) {
		if (tag !== "h1") return [];
		const textLen = headingText.length;
		if (fontSize >= OversizedH1FontPx && textLen >= OversizedH1MinChars) {
			return [
				{
					id: "oversized-h1",
	...
	const TextOverflowSkipTags = new Set([
		"pre",
		"code",
		"textarea",
		"svg",
	...
	function checkElementTextOverflowDom(el) {
		const tag = el.tagName.toLowerCase();
		if (TextOverflowSkipTags.has(tag)) return [];
	...
	if (IsBrowser) {
		...
		const BrandColor = "oklch(84% 0.19 80.46)";
		const BrandColorHover = "oklch(74% 0.18 80)";
		const LabelInk = "oklch(4% 0.004 95)";
		const LabelBg = BrandColor;
		const OutlineColor = BrandColor;
  ...
  `;
		...
		for (const ap of Antipatterns) {
			TypeLabels[ap.id] = ap.name.toLowerCase();
			RuleCategory[ap.id] = ap.category || "quality";
		}
		...
		const highlight = (el, findings) => {
			...
			const hasSlop = findings.some((f) => RuleCategory[f.type || f.id] === "slop");
			...
			const entries = findings.map((f) => {
				const name = TypeLabels[f.type || f.id] || f.type || f.id;
				const prefix = RuleCategory[f.type || f.id] === "slop" ? "\u2726 " : "";
			...
			const onMouseEnter = () => {
				...
				outline.style.outlineColor = BrandColorHover;
				label.style.background = BrandColorHover;
				if (cycleMode) {
					updateCycleText();
				} else {
		...
		const showPageBanner = (findings) => {
			...
			for (const f of findings) {
				const prefix = RuleCategory[f.type] === "slop" ? "\u2726 " : "";
				const tag = document.createElement("span");
				tag.textContent = `${prefix}${TypeLabels[f.type] || f.type}: ${f.detail}`;
		...
		function collectVisualContrastCandidates(options = {}) {
			...
			for (const el of document.querySelectorAll("*")) {
				...
				if (SafeTags.has(tag) && !isStyledButton) continue;
		...
		function collectBrowserFindings() {
			...
			const _disabled = ExtensionMode ? window.__IMPECCABLE_CONFIG__?.disabledRules || [] : [];
		...
		function renderBrowserFindings(collected, options = {}) {
			...
			if (!ExtensionMode) printSummary(allFindings);
