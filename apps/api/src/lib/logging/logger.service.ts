...
export class LoggerService extends Logger {
    ...
    constructor(context = 'CermontApp') {
        ...
        this.jsonFormat = (process.env.LOG_FORMAT || '').toLowerCase() === 'json' ||
    ...
    audit(action: string, userId: string, resource: string, details?: Record<string, unknown>): void {
        this.write('log', `[AUDIT] ${action} - User: ${userId}, Resource: ${resource}`, 'Audit', {
            action,
            userId,
            resource,
            details,
    ...
    performance(
        ...
        this.write(level, `[PERF] ${label} - ${durationMs}ms`, 'Performance', { durationMs, threshold, ...meta }, undefined, 'performance');
    }
    ...
    http(method: string, url: string, statusCode: number, durationMs: number, userId?: string): void {
        ...
        this.write(level, `[HTTP] ${method} ${sanitizeUrl(url)} ${statusCode} - ${durationMs}ms`, 'HTTP', {
            method,
            url: sanitizeUrl(url),
            statusCode,
            durationMs,
    ...
    private async writeToFile(entry: LogEntry): Promise<void> {
        ...
        if (this.fileConfig.mode === 'single') {
            if (!this.fileInitPromise) {
                this.fileInitPromise = mkdir(dirname(this.fileConfig.filePath), { recursive: true }).then(() => undefined);
            }
        ...
        const targetPath = entry.level === 'error' ? this.fileConfig.errorFilePath : this.fileConfig.infoFilePath;
