 ...
 * @layer Domain
 */
import { TecnicoDisponibilidad, DisponibilidadLevel, TecnicoEspecialidad, EspecialidadType } from '../value-objects';
...
export class TecnicoEntity {
    ...
    get ordenesActivas(): number { return this.props.ordenesActivas; }
    get ordenesCompletadas(): number { return this.props.ordenesCompletadas; }
    get calificacionPromedio(): number | undefined { return this.props.calificacionPromedio; }
    ...
    get isAvailableForAssignment(): boolean {
        return this.active && this.disponibilidad.canBeAssigned && this.ordenesActivas < 3;
    }

    // Factory Methods
    static create(props: Omit<TecnicoProps, 'id' | 'ordenesActivas' | 'ordenesCompletadas' | 'createdAt' | 'updatedAt'>): TecnicoEntity {
        const now = new Date();
    ...
    changeDisponibilidad(newDisponibilidad: DisponibilidadLevel): void {
        ...
        if (!current.canTransitionTo(newDisponibilidad)) {
            throw new Error(`Transición inválida de ${current.value} a ${newDisponibilidad}`);
        }
    ...
    completeOrden(): void {
        ...
        if (this.props.ordenesActivas === 0 && this.props.disponibilidad === 'ocupado') {
            this.setDisponible();
        }
        this.props.updatedAt = new Date();
    }
