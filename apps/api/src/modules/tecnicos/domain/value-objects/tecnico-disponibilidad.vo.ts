 ...
 * @layer Domain
 */
export type DisponibilidadLevel = 'disponible' | 'ocupado' | 'vacaciones' | 'baja';

export class TecnicoDisponibilidad {
    private static readonly VALID_TRANSITIONS: Record<DisponibilidadLevel, DisponibilidadLevel[]> = {
    ...
    canTransitionTo(newState: DisponibilidadLevel): boolean {
        return TecnicoDisponibilidad.VALID_TRANSITIONS[this._value].includes(newState);
    }
