...
export class TecnicoEspecialidad {
    private static readonly VALID_TYPES: EspecialidadType[] = [
        'electrico', 'mecanico', 'soldadura', 'altura', 'hes', 'general'
    ];
    ...
    canPerform(requiredEspecialidades: EspecialidadType[]): boolean {
        return requiredEspecialidades.every(req =>
            this._values.includes(req) || this._values.includes('general')
        );
    }
    ...
    equals(other: TecnicoEspecialidad): boolean {
        return JSON.stringify(this._values.sort()) === JSON.stringify(other._values.sort());
    }
