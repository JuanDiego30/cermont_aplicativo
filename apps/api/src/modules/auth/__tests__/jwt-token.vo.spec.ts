...
describe('JwtToken VO', () => {
  ...
  it('fromString: retorna null si verify falla', () => {
    const jwtService = { verify: jest.fn(() => { throw new Error('bad'); }) } as unknown as JwtService;
  ...
  it('fromString: calcula expiración correctamente', () => {
    ...
    const jwtService = {
      verify: jest.fn(() => ({ userId: 'u1', email: 'u1@example.com', role: 'tecnico', exp })),
  ...
  it('fromString: sin exp -> expiresAt null e isExpired false', () => {
    const jwtService = {
      verify: jest.fn(() => ({ userId: 'u1', email: 'u1@example.com', role: 'tecnico' })),
    } as unknown as JwtService;
  ...
  it('equals + toString: comparan por value', () => {
    const jwtService = { sign: jest.fn(() => 'tkn') } as unknown as JwtService;
    const a = JwtToken.create(jwtService, { userId: 'u1', email: 'u1@example.com', role: 'admin' });
    const b = JwtToken.create(jwtService, { userId: 'u1', email: 'u1@example.com', role: 'admin' });
