...
export class ChecklistsService {
  ...
  async createFromTemplate(
    dto: { ejecucionId: string; tipo: string },
    userId: string,
  ) {
    const templates = await this.repository.findByTipo(dto.tipo);
    if (!templates || templates.length === 0)
      throw new NotFoundException("No hay templates para este tipo");
  ...
  async toggleItem(itemId: string, userId?: string) {
    ...
    throw new Error(
      "Metodo toggleItem requiere checklistId en nueva arquitectura",
    );
  }
  ...
  async updateItem(
    ...
    throw new Error(
      "Metodo updateItem requiere checklistId en nueva arquitectura",
    );
  }
  ...
  async updateChecklistItem(
    ...
    throw new Error(
      "Metodo updateChecklistItem requiere checklistId en nueva arquitectura",
    );
  }
  ...
  async completar(id: string, userId: string) {
    ...
    return {
      ...
      data: ChecklistMapper.toResponseDto(checklist),
    };
  }
  ...
  async getStatistics(ejecucionId: string) {
    ...
    return {
      total,
      completas,
      porcentaje: total > 0 ? (completas / total) * 100 : 0,
    };
  }
