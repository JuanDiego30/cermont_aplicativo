...
export class DashboardController {
    ...
    @ApiResponse({ status: 200, description: 'Órdenes recientes obtenidas exitosamente' })
    @ApiResponse({ status: 401, description: 'No autorizado' })
    ...
    @ApiResponse({ status: 200, description: 'KPIs consolidados obtenidos exitosamente' })
    @ApiResponse({ status: 401, description: 'No autorizado' })
    @ApiResponse({ status: 403, description: 'Acceso denegado. Solo supervisores y admins' })
    ...
    @ApiResponse({ status: 200, description: 'Desglose de costos obtenido exitosamente' })
    @ApiResponse({ status: 401, description: 'No autorizado' })
    @ApiResponse({ status: 403, description: 'Acceso denegado. Solo supervisores y admins' })
    ...
    @ApiQuery({ name: 'desde', required: true, example: '2024-01-01', description: 'Fecha inicio (YYYY-MM-DD)' })
    @ApiQuery({ name: 'hasta', required: true, example: '2024-12-31', description: 'Fecha fin (YYYY-MM-DD)' })
    @ApiQuery({ 
        name: 'granularidad', 
        required: false, 
        enum: GranularidadTendencia,
    ...
    @ApiResponse({ status: 200, description: 'Tendencias obtenidas exitosamente' })
    @ApiResponse({ status: 400, description: 'Fechas inválidas o rango excesivo (máx 365 días)' })
    @ApiResponse({ status: 401, description: 'No autorizado' })
    @ApiResponse({ status: 403, description: 'Acceso denegado. Solo supervisores y admins' })
    async getTendencias(
        @Query('desde') desde: string,
        @Query('hasta') hasta: string,
        @Query('granularidad') granularidad: GranularidadTendencia = GranularidadTendencia.MES,
