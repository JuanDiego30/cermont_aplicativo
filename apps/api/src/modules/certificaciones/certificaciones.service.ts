...
export class CertificacionesService {
    ...
    async getCertificacionesTecnico(tecnicoId: string): Promise<CertificacionResponseDto[]> {
        ...
        return certificados.map((cert: { id: string; tipo: string; nombre: string; entidad: string; numero: string; fechaExpedicion: Date; fechaVencimiento: Date; archivo: string | null; observaciones: string | null }) => {
    ...
    async validarCertificaciones(dto: ValidarCertificacionesDto): Promise<ValidacionResultDto> {
        ...
        const invalid: Array<{ id: string; nombre: string; tipo: string; razon: string }> = [];
        const alerts: Array<{ id: string; tipo: string; diasRestantes: number; nivel: string }> = [];
        ...
        for (const tecnicoId of dto.tecnicosIds) {
            ...
            if (dto.tiposRequeridos && dto.tiposRequeridos.length > 0) {
                ...
                if (!tieneVigente) {
                    ...
                    valid.push(...certificaciones.filter(
                        (c) => c.estadoVigencia !== EstadoVigenciaType.VENCIDA,
                    ));
