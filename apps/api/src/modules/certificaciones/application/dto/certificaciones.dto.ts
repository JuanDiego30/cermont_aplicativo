...
import { IsString, IsNotEmpty, IsDateString, IsOptional, IsEnum, IsUrl } from 'class-validator';
import { TipoCertificacionTecnico, TipoCertificacionEquipo } from '../../domain/value-objects/tipo-certificacion.vo';
...
export class CreateCertificacionTecnicoDto {
    ...
    @ApiProperty({ enum: TipoCertificacionTecnico, description: 'Tipo de certificación' })
    ...
    @ApiProperty({ description: 'Número de certificado', example: 'CERT-2024-001' })
    @IsString()
    @IsNotEmpty()
...
export class CreateCertificacionEquipoDto {
    ...
    @ApiProperty({ enum: TipoCertificacionEquipo, description: 'Tipo de certificación' })
...
export class CertificacionResponseDto {
    ...
    @ApiPropertyOptional({ description: 'Nivel de alerta', enum: ['INFO', 'WARNING', 'CRITICAL'] })
...
export class ValidarCertificacionesDto {
    ...
    @ApiPropertyOptional({ description: 'IDs de equipos a validar', type: [String] })
    @IsOptional()
    @IsString({ each: true })
    equiposIds?: string[];

    @ApiPropertyOptional({ description: 'Tipos de certificación requeridos', type: [String] })
    @IsOptional()
    @IsString({ each: true })
