...
import { Ejecucion, EjecucionProps } from '../../domain/entities/ejecucion.entity';
...
import { ExecutionStatus, ExecutionStatusEnum } from '../../domain/value-objects/execution-status.vo';
...
export class EjecucionRepository implements IEjecucionRepository {
  ...
  async findByTecnico(tecnicoId: string, status?: ExecutionStatus): Promise<Ejecucion[]> {
    const where: any = {
  ...
  async saveTimeLog(log: TimeLog): Promise<TimeLog> {
    ...
    this.logger.warn('TimeLog persistence not implemented - storing in aggregate only');
    return log;
  }
  ...
  async saveActivityLog(log: ActivityLog): Promise<ActivityLog> {
    this.logger.warn('ActivityLog persistence not implemented - storing in aggregate only');
    return log;
  }
  ...
  async getEvidences(ejecucionId: EjecucionId): Promise<Evidence[]> {
    ...
    return evidencias.map((e) => Evidence.fromPersistence({
      id: e.id,
