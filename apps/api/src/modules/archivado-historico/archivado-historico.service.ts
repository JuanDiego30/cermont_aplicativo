...
export class ArchivadoHistoricoService {
    ...
    async archivarAutomatico(): Promise<ResultadoArchivadoDto> {
        ...
        this.logger.log(`Iniciando archivado automático para órdenes anteriores a ${fechaLimite.toISOString()}`);
    ...
    async getEstadisticas(): Promise<EstadisticasArchivoDto> {
        ...
        return {
            ...
            totalExportaciones: fs.existsSync(this.EXPORT_DIR) ? fs.readdirSync(this.EXPORT_DIR).length : 0,
    ...
    async restaurarOrden(ordenId: string): Promise<OrdenArchivadaResponseDto> {
        ...
        return this.mapOrdenToResponse({ ...orden, observaciones: nuevasObservaciones });
    }
    ...
    private async generarCSV(ordenes: any[], rutaArchivo: string): Promise<void> {
        const headers = ['ID', 'Número', 'Descripción', 'Cliente', 'Estado', 'Fecha Creación', 'Fecha Cierre', 'Costo Real'];
