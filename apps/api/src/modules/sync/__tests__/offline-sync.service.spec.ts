...
describe("OfflineSyncService", () => {
  ...
  describe("buildOfflineChecklist", () => {
    ...
    it("debería lanzar error si ejecución no existe", async () => {
      ...
      await expect(service.buildOfflineChecklist("no-existe")).rejects.toThrow(
        BadRequestException,
      );
    });
  });
  ...
  describe("validateOfflineIntegrity", () => {
    ...
    it("debería detectar items duplicados", async () => {
      ...
      await expect(
        service.validateOfflineIntegrity(payloadConDuplicados),
      ).rejects.toThrow("Payload contiene items duplicados");
    });
    ...
    it("debería detectar estados inválidos", async () => {
      ...
      await expect(
        service.validateOfflineIntegrity(payloadConEstadoInvalido),
    ...
    it("debería detectar items que no existen en BD", async () => {
      ...
      await expect(
        service.validateOfflineIntegrity(validPayload),
      ).rejects.toThrow("Items no encontrados en BD");
    });
  });
