...
export class CreateKitDto {
    ...
    @ApiProperty({ example: 'Herramientas y equipos para inspección de líneas de vida verticales' })
    @IsString()
    @IsNotEmpty()
    descripcion!: string;
    ...
    @ApiProperty({ type: [String], example: ['Formato Inspección Líneas de Vida Vertical'] })
    @IsArray()
    @ArrayMinSize(1)
    @IsString({ each: true })
    documentos!: string[];

    @ApiProperty({ type: [String], example: ['Verificar estado general del cable'] })
    @IsArray()
    @ArrayMinSize(1)
    @IsString({ each: true })
