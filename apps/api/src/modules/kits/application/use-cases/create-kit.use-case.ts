...
export class CreateKitUseCase {
  ...
  private buildItems(dto: CreateKitDto): Array<{
    ...
    if (dto.herramientas) {
      for (const h of dto.herramientas as Array<{ nombre: string; cantidad?: number; certificacion?: boolean }>) {
        items.push({
          nombre: h.nombre,
    ...
    // From legacy equipos
    if (dto.equipos) {
      for (const e of dto.equipos as Array<{ nombre: string; cantidad?: number; certificacion?: boolean }>) {
        items.push({
          nombre: e.nombre,
