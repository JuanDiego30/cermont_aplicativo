...
export class CreateClienteDto {
    @ApiProperty({ description: 'Razón social', example: 'SIERRACOL ENERGY ARAUCA LLC' })
    @IsString()
    @IsNotEmpty()
    razonSocial!: string;
    ...
    @ApiPropertyOptional({ type: [CreateContactoDto], description: 'Contactos del cliente' })
    @IsOptional()
    @IsArray()
    ...
    @ApiPropertyOptional({ type: [CreateUbicacionDto], description: 'Ubicaciones del cliente' })
    @IsOptional()
    @IsArray()
    @ValidateNested({ each: true })
