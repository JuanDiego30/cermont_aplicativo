  ...
  implements OnGatewayInit, OnGatewayConnection, OnGatewayDisconnect
{
  ...
  handleConnection(client: Socket): void {
    ...
    this.logger.debug(`Total de sockets para usuario ${userId}: ${this.userSockets.get(userId)!.size}`);
  ...
  handleDisconnect(client: Socket): void {
    ...
    for (const [userId, sockets] of this.userSockets.entries()) {
      if (sockets.has(client.id)) {
        ...
        this.logger.log(`Cliente ${client.id} desconectado de usuario ${userId}`);
        break;
      }
    }
  }
  ...
  handleSubscribe(
    ...
    client.join(`user:${userId}`);
    this.logger.log(`Cliente ${client.id} suscrito a alertas de usuario ${userId}`);
  ...
  handleUnsubscribe(
    ...
    this.logger.log(`Cliente ${client.id} desuscrito de alertas de usuario ${userId}`);
  ...
  async handleMarcarLeida(
    ...
    this.logger.log(`Alerta ${data.alertaId} marcada como leída por usuario ${data.userId}`);
  }
  ...
  handleAlertaEnviada(payload: AlertaEnviadaEvent): void {
    ...
    this.logger.debug(`Notificación de alerta enviada a usuario ${userId} por WebSocket`);
  }
  ...
  handleAlertaFallida(payload: AlertaFallidaEvent): void {
    ...
    this.logger.debug(`Notificación de fallo de alerta enviada a usuario ${userId}`);
  }
  ...
  getConnectionStats(): {
    ...
    const users = Array.from(this.userSockets.entries()).map(([userId, sockets]) => ({
      userId,
