...
export class AlertasService {
  ...
  async checkActasSinFirmar() {
    ...
    for (const acta of actasPendientes) {
      ...
      await this.prisma.acta.update({
        ...
        data: {
          alertaEnviada: true,
          diasSinFirmar: this.calcularDias(acta.fechaEmision),
        },
      });
    }

    this.logger.log(
      `${actasPendientes.length} alertas de actas sin firmar generadas`,
    );
  }
  ...
  async checkSESPendientes() {
    ...
    for (const ses of sesPendientes) {
      ...
      await this.prisma.sES.update({
        ...
        data: {
          alertaEnviada: true,
          diasSinAprobar: this.calcularDias(ses.fechaCreacion),
        },
      });
    }

    this.logger.log(
      `${sesPendientes.length} alertas de SES pendientes generadas`,
    );
  }
  ...
  async checkFacturasVencidas() {
    ...
    for (const factura of facturasVencidas) {
      ...
      const prioridad: PrioridadAlerta =
        diasVencidos > 30
          ? "critical"
          : diasVencidos > 15
            ? "error"
    ...
    this.logger.log(
      `${facturasVencidas.length} alertas de facturas vencidas generadas`,
    );
  }
  ...
  async checkPropuestasSinRespuesta() {
    ...
    this.logger.log(
      `${propuestasPendientes.length} alertas de propuestas sin respuesta generadas`,
    );
  }
  ...
  async getResumenAlertas() {
    const [total, criticas, noLeidas, porTipo] = await Promise.all([
      ...
      this.prisma.alertaAutomatica.count({
        where: { resuelta: false, prioridad: "critical" as any },
      }),
      this.prisma.alertaAutomatica.count({
  ...
  private async crearAlerta(data: {
    ...
    if (existente) {
      this.logger.debug(
        `Alerta ${data.tipo} ya existe para orden ${data.ordenId}`,
      );
      return existente;
    }
