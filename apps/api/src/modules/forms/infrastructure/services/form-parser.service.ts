...
export class FormParserService {
    ...
    async parseFromPDF(buffer: Buffer, filename: string): Promise<ParsedFormSchema> {
        this.logger.log('Parseando PDF para extraer estructura de formulario', { filename });
        ...
        try {
            ...
            this.logger.error('Error parseando PDF', { filename, error: err.message });
    ...
    async parseFromExcel(buffer: Buffer, filename: string): Promise<ParsedFormSchema> {
        this.logger.log('Parseando Excel para extraer estructura de formulario', { filename });
        ...
        try {
            ...
            const err = error as Error;
            this.logger.error('Error parseando Excel', { filename, error: err.message });
    ...
    private inferFormType(text: string): string {
        ...
        if (/checklist|check list|lista de verificación/i.test(lower)) return 'CHECKLIST';
